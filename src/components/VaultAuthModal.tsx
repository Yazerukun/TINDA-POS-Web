...
import { KeyRound, ShieldCheck, UserCheck, Delete, ArrowRight, Lock, Clock, Fingerprint, WifiOff, ServerCog, Banknote, ChevronLeft } from 'lucide-react'
...
export function VaultAuthModal({ isOpen, onAuthenticated }: VaultAuthModalProps): React.JSX.Element | null {
  ...
  return (
    <div className="fixed inset-0 z-50 flex bg-obsidian-950/85 backdrop-blur-2xl">
      {/* Ambient backdrop: grid veil + aurora glows */}
      ...
      <div className="relative w-full lg:max-w-none grid lg:grid-cols-2">
        <BrandPanel terminalId={TERMINAL_ID} />

        {/* Auth column */}
        <main className="flex flex-col items-center justify-center min-h-screen w-full p-6 sm:p-10">
          <div className="w-full max-w-md glass-vault rounded-3xl border border-gold/25 shadow-vault text-stone-100 animate-fade-in">
            {/* Card header: compact brand lockup + step pill */}
            <div className="flex items-center justify-between px-7 pt-6">
              <div className="flex items-center gap-2.5">
                <div className="relative flex h-9 w-9 items-center justify-center rounded-lg bg-gradient-to-b from-amber-200/20 via-gold/15 to-transparent border border-gold/40 shadow-glow-gold">
                  <span className="font-serif text-base font-bold tracking-widest text-gold-light">T</span>
                </div>
                <div className="leading-tight">
                  <p className="font-serif text-sm font-bold tracking-[0.18em] uppercase text-stone-100">TINDA POS</p>
                  <p className="font-mono text-[8px] tracking-[0.24em] uppercase text-gold-muted font-medium">Secure Access</p>
                </div>
              </div>
              <span className="inline-flex items-center gap-1.5 rounded-full glass-pill px-3 py-1.5 font-mono text-[10px] tracking-[0.2em] uppercase text-gold-light">
              ...
              {step === 'PIN' ? (
                <div className="animate-fade-in">
                  ...
                  <div className="mb-6">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <UserCheck className="h-3.5 w-3.5 text-gold-light/70" />
                    </div>
                    ...
                    <div className="grid grid-cols-3 gap-2.5">
                      {CASHIER_PROFILES.map((profile) => {
                        ...
                        return (
                          ...
                          >
                            <div
                              className={`relative flex h-10 w-10 items-center justify-center rounded-full text-xs font-serif font-bold transition-all ${
                                isSelected
                  ...
                  <div className="flex flex-col items-center mb-6">
                    ...
                    {pinError ? (
                      <span className="text-[11px] font-mono text-red-400 mt-2.5 tracking-wider">
                        Invalid security code — please try again.
                      </span>
                    ) : (
                      <span className="text-[10px] font-mono text-stone-500 mt-2.5 tracking-wider">
                  ...
                  {/* Keypad */}
                  <div className="grid grid-cols-3 gap-2.5 max-w-[270px] mx-auto mb-5">
                    {['1', '2', '3', '4', '5', '6', '7', '8', '9'].map((digit) => (
                      <button
                        key={digit}
                        onClick={() => handleKeyPress(digit)}
                        className="btn-press flex items-center justify-center h-13 rounded-2xl bg-zinc-950/40 hover:bg-gold/[0.1] border border-white/[0.07] hover:border-gold/40 text-stone-100 font-mono text-xl font-semibold transition-all duration-200 active:scale-95 shadow-sm py-3.5"
                      >
                        {digit}
                      </button>
                    ))}
                    <button
                      onClick={handleClear}
                      className="btn-press flex items-center justify-center h-13 rounded-2xl bg-zinc-950/40 hover:bg-white/[0.06] border border-white/[0.07] text-[10px] font-mono tracking-widest text-stone-400 hover:text-stone-200 transition-all uppercase py-3.5"
                    >
                      Clr
                    </button>
                    <button
                    ...
                    <button
                      ...
                      className="btn-press flex items-center justify-center h-13 rounded-2xl bg-zinc-950/40 hover:bg-white/[0.06] border border-white/[0.07] text-stone-400 hover:text-stone-200 transition-all py-3.5"
                    >
                      <Delete className="h-5 w-5" />
                    </button>
                  </div>

                  <p className="text-center font-mono text-[10px] text-stone-500 tracking-wider flex items-center justify-center gap-1.5">
                ...
                <div className="animate-fade-in">
                  ...
                  <div className="flex flex-col gap-2.5">
                    ...
                    <button
                      onClick={() => setStep('PIN')}
                      className="w-full py-2.5 text-[11px] font-mono tracking-wider text-stone-400 hover:text-stone-200 transition-colors uppercase text-center flex items-center justify-center gap-1.5"
                    >
          ...
          <p className="mt-5 font-mono text-[10px] tracking-[0.2em] uppercase text-stone-600 flex items-center gap-2">
